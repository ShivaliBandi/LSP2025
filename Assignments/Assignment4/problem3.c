...
int main(int argc,char *argv[])
{
    if(argc!=3)
    {
        printf("ERROR:Invalid number of arguments\nSYNTAX -> problem3 <source file name> <destination file name>\n");
        exit(-1);
    }
    ...
    while((readcount=pread(fdsource,buffer,10,offset))!=0)
    {
        
        writecount=pwrite(fddestination,buffer,strlen(buffer),offset);
        offset += readcount;
